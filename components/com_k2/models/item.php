...
class K2ModelItem extends K2Model
{
	...
	function prepareItem($item, $view, $task)
	{
		...
		if ($item->category->id == 13) {
			
			$dir = $_SERVER['DOCUMENT_ROOT'];
      $imgDir = JURI::base(true).'/images/fabrics/'.$item->alias;
      $src = $dir.$imgDir;
	...
	function getVotesNum($itemID = NULL)
	{
		...
		if($rating_count%10 == 1 && $rating_count != 11)
		{
			$result = "(".$rating_count." ".JText::_('K2_VOTE').")";
		}
		elseif ($rating_count%10 > 1 && $rating_count%10 <= 4 && $rating_count < 11 && $rating_count > 19)
		{
			$result = "(".$rating_count." ".JText::_('K2_VOTESES').")";
		}
		else {
