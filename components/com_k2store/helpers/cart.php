...
class K2StoreHelperCart
{
	...
	public function __construct() {
		...
		$fabricid = JFactory::getSession()->get('fabric_id');
		if (!isset($cart) || !is_array($cart) ) {
			JFactory::getSession()->set('k2store_cart', array());
		}
		if(!isset($fabricid) || !is_array($fabricid)){
			JFactory::getSession()->set('fabric_id', array());
		}
	}


public function add($product_id, $qty=1,  $options = array(), $fabrics = array()) {
	$cart = JFactory::getSession()->get('k2store_cart');
	$fabricid = JFactory::getSession()->get('fabric_id');
	if (!$options) {
		$key = (int)$product_id;
	...
	if (!$fabrics) {
		...
		$fkey = (int)$product_id . ':' . base64_encode(serialize($fabrics));
	}
	...
	if ((int)$qty && ((int)$qty > 0)) {
		...
		if (!isset($fabricid[$fkey])) {
			$fabricid[$fkey] = (int)$qty;

		} else {
			$fabricid[$fkey] += (int)$qty;
		}
		// if (!isset($fabricid[$product_id])) {
		// }
		JFactory::getSession()->set('k2store_cart', $cart);
		JFactory::getSession()->set('fabric_id', $fabricid);
	}
	...
	foreach ($cartitems as $cartitem)
	{
		if ($productItem = K2StoreHelperCart::getItemInfo($cartitem['product_id']))
		{
			...
			if(isset($price_override) && !empty($price_override)) {
				$price = 0/*$price_override->product_price*/;
			}
