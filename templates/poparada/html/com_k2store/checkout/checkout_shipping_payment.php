...
<div id='onCheckoutPayment_wrapper'>
	...
	if ($this->plugins)
	{
		foreach ($this->plugins as $plugin)
		{
			...
			if(!empty($title)) {
				echo JText::_($title).'</label></div>';
			} else {

				echo JText::_($plugin->name ).'</label></div>';
			}
			?>
...
(function($){
    $('textarea#address_1').attr('placeholder', 'улица Водогонная дом 9');
    $('input#zip').attr('placeholder', '03040');
    $('input#company').attr('placeholder', 'Poparada');
})(k2store.jQuery);
...
<div class="buttons">
  <div class="right">
    <input type="button" value="<?php echo JText::_('K2STORE_CHECKOUT_CONTINUE'); ?>" id="button-payment-method" class="button btn btn-smaller" />
  </div>
</div>