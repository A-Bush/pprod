...
class K2StorePluginBase extends JPlugin
{
    /**
     ...
     * @access protected
     */
    function _getLayout($layout, $vars = false, $plugin = '', $group = 'k2store' )
    {
        if (empty($plugin))
        {
    ...
    /**
     * Make the standard K2Store Tables avaiable in the plugin
     */
    protected function includeK2StoreTables() {
    	// Include K2Store Tables Classes
    	JTable::addIncludePath(JPATH_ADMINISTRATOR . '/components/com_k2store/tables');
    }

    /**
     * Include a particular K2Store Model
     * @param $name the name of the mode (ex: products)
     */
    protected function includeK2StoreModel($name, $admin=true) {
    	if ($admin)
    		$base_path=JPATH_ADMINISTRATOR;
    	else
    		$base_path=JPATH_SITE;
    	JModelAdmin::addIncludePath($base_path.'/components/com_k2store/models/'.strtolower($name));
    }
     ...
     */
    protected function includeCustomModel($name, $plugin = '', $group = 'k2store') {
    	if (empty($plugin)) {
    		$plugin = $this -> _element;
    	}

    	if (!class_exists('K2StoreModel' . $name))
    		JLoader::import('plugins.' . $group . '.' . $plugin . '.' . $plugin . '.models.' . strtolower($name), JPATH_SITE);
    }

    /**
     * add a user-defined table to list of available tables (including the k2store tables
     * @param $plugin the name of the plugin in which the table is stored
     * @param $group the group of the plugin
     */
    protected function includeCustomTables($plugin = '', $group = 'k2store') {
     ...
     * @param $group the group of the plugin
     */
    protected function includeCustomView($name, $plugin = '', $group = 'k2store') {
    	if (empty($plugin)) {
    		$plugin = $this -> _element;
    	}
    	if (!class_exists('K2StoreView' . $name))
